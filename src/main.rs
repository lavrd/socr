...
fn main() {
    ...
    let (srv, srv_evt_s) = server::Server::new();
    let (cons, cons_evt_s) = consensus::Consensus::new();

    let stop_cons = consensus::spawn(cons, cons_evt_s.clone(), srv_evt_s.clone());
    let stop_srv = server::spawn(srv, srv_evt_s, cons_evt_s);
